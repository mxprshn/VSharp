...
module internal Simplification =
    ...
    let private solver = context.MkSolver()

    let rec private boolExprToGraph (cache : Dictionary<BoolExpr, Node>) (expr : BoolExpr) : Graph =
        if expr.IsAnd then
    ...
    let rec private graphToBoolExpr graph =
        match graph with
    ...
    let private checkRedundancy leaf critical =
